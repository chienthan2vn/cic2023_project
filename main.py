...
from fcm import FCM
from preprocess import _preprocess_cic2023_mini, _preprocess_cic2023_mini_for_pca

print("=== ÁP DỤNG FCM TRÊN CIC 2023 MINI ===\n")

x, y_encoded = _preprocess_cic2023_mini()
...
x_pca, y_encoded = _preprocess_cic2023_mini_for_pca(dim=2)
# 3. FCM Euclidean trên dữ liệu PCA (2D)
print("\n3. FCM EUCLIDEAN - PCA Features (2D):")
fcm_euc_pca = FCM(n_clusters=5, m=2, max_iter=50, tol=1e-4, distance_metric='euclidean')
fcm_euc_pca.fit(x_pca)
labels_euc_pca = fcm_euc_pca.predict(x_pca)

# 4. FCM Canberra trên dữ liệu PCA (2D)
print("\n4. FCM CANBERRA - PCA Features (2D):")
fcm_can_pca = FCM(n_clusters=5, m=2, max_iter=50, tol=1e-4, distance_metric='canberra')
fcm_can_pca.fit(x_pca)
labels_can_pca = fcm_can_pca.predict(x_pca)
...
ari_euc_pca = adjusted_rand_score(y_encoded, labels_euc_pca)
ari_can_pca = adjusted_rand_score(y_encoded, labels_can_pca)
...
print(f"- FCM Euclidean (PCA):  {ari_euc_pca:.4f}")
print(f"- FCM Canberra (PCA):   {ari_can_pca:.4f}")

# Silhouette Score
sil_euc_full = silhouette_score(x, labels_euc_full)
sil_can_full = silhouette_score(x, labels_can_full)
sil_euc_pca = silhouette_score(x_pca, labels_euc_pca)
sil_can_pca = silhouette_score(x_pca, labels_can_pca)
...
print(f"- FCM Euclidean (PCA):  {sil_euc_pca:.4f}")
print(f"- FCM Canberra (PCA):   {sil_can_pca:.4f}")
...
print("FCM Euclidean (PCA):", np.bincount(labels_euc_pca))
print("FCM Canberra (PCA):", np.bincount(labels_can_pca))

# Tìm thuật toán tốt nhất
best_ari = max(ari_euc_full, ari_can_full, ari_euc_pca, ari_can_pca)
best_sil = max(sil_euc_full, sil_can_full, sil_euc_pca, sil_can_pca)

print(f"\n🏆 THUẬT TOÁN TỐT NHẤT:")
algorithms = [
    ("FCM Euclidean (Full)", ari_euc_full, sil_euc_full),
    ("FCM Canberra (Full)", ari_can_full, sil_can_full),
    ("FCM Euclidean (PCA)", ari_euc_pca, sil_euc_pca),
    ("FCM Canberra (PCA)", ari_can_pca, sil_can_pca)
]
